...
const GlobalChat = () => {
    ...
    {rooms.length > 0 ? rooms.map(r => (
        <RoomItem key={r.roomId} onClick={() => { setSelectedRoomId(r.roomId); setView('chat'); }}>
            {/* ✅ 수정: 프로필 URL이 있으면 img 태그를, 없으면 기본 이모지 표시 */}
            <div className="room-icon">
                {r.otherUserProfileUrl ? (
                    <img src={r.otherUserProfileUrl} alt="profile" />
                ) : (
                    "👤"
                )}
            </div>
...
const RoomItem = styled.div` 
    ...
    .room-icon { 
        ...
        font-size: 20px;
        flex-shrink: 0;
        overflow: hidden; /* ✅ 추가: 이미지가 원을 벗어나지 않도록 함 */
    ...
    .room-info { 
        flex: 1; 
        text-align: left;
        b { font-size: 14px; color: #1e293b; display: block; } 
        p { margin: 4px 0 0; font-size: 12px; color: #64748b; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 200px; } 
    } 
`;

const UnreadBadge = styled.div` background: #ef4444; color: white; border-radius: 999px; padding: 2px 6px; font-size: 10px; font-weight: bold; min-width: 20px; height: 20px; display: flex; align-items: center; justify-content: center; flex-shrink: 0; `;
...
const MsgBubble = styled.div` align-self: ${props => props.$isMe ? 'flex-end' : 'flex-start'}; max-width: 80%; .bubble-row { display: flex; align-items: flex-end; flex-direction: ${props => props.$isMe ? 'row-reverse' : 'row'}; } .text { padding: 8px 12px; border-radius: 12px; font-size: 13px; background: ${props => props.$isMe ? '#6366f1' : 'white'}; color: ${props => props.$isMe ? 'white' : 'black'}; border: ${props => props.$isMe ? 'none' : '1px solid #e2e8f0'}; word-break: break-all; opacity: ${props => props.$isTemp ? 0.6 : 1}; } .meta { display: flex; flex-direction: column; align-items: ${props => props.$isMe ? 'flex-end' : 'flex-start'}; margin: 0 5px; } .read-status { color: #6366f1; font-size: 10px; font-weight: bold; margin-bottom: 2px; } .time { font-size: 9px; color: #94a3b8; } .sender-name { font-size: 11px; color: #64748b; margin-bottom: 4px; } `;
